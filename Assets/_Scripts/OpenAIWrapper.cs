...
public class OpenAIWrapper : MonoBehaviour
{
    ...
    public async Task<byte[]> RequestTextToSpeech(string text)
    {
        ...
        //haetaan ApiKey, joka on tallennettu tietokoneelle

        using (StreamReader sr = new StreamReader(System.Environment.GetFolderPath(System.Environment.SpecialFolder.UserProfile) + "/.openai/auth.json"))
        {
